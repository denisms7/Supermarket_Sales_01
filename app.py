...
html.Div([
        ...
        ], style={'background-color': '#111111', 'height':'100%', 'padding': '10px'}),
], className='col-md-2 p-1 h-auto'),

html.Div([
        html.Div([
            html.Div([dcc.Graph(id='fig_cidade')], className='col-6 p-1'),
            html.Div([dcc.Graph(id='fig_pagamento')], className='col-6 p-1'),
        ], className='row'),

        html.Div([
            html.Div([dcc.Graph(id='fig_valor_tempo')], className='col-12 p-1')
        ], className='row'),

        html.Div([
            html.Div([dcc.Graph(id='fig_produto')], className='col-12 p-1 h-25')
        ], className='row'),
...
def renderizar_graficos(check_cidades, check_agrupador):
    ...
    df_tempo = df_filtro.groupby(['Date', 'City'])[check_agrupador].apply(operador).to_frame().reset_index()
    ...
    fig_valor_tempo = px.bar(df_tempo, x='Date', y=check_agrupador, color='City')
    ...
    fig_valor_tempo.update_layout(template='plotly_dark', height=250, transition={"duration": 400})
